...
- (id)initWithButtonTitles:(NSArray *) buttonTitles stratingPoint:(CGFloat)startPoint
{
    self = [self init];
    if (self) {
        _buttonTitles = buttonTitles;
...
- (void) renderButtons
{
    ...
    for (NSUInteger i = 0; i < [_buttonTitles count]; i++) {
        ...
        linelbl.frame=CGRectMake(self.buttonWrapper.bounds.origin.x, self.buttonWrapper.bounds.origin.y+(34*i), self.buttonWrapper.bounds.size.width, 1);
        [linelbl setBackgroundColor:[UIColor whiteColor]];
        [self.buttonWrapper addSubview:linelbl];
